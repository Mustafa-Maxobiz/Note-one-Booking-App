...
<div class="page-header mb-4">
    <div class="row align-items-center">
        <div class="col-md-8">
            ...
            <p class="page-subtitle">Manage your teaching sessions and student bookings</p>
        </div>
            ...
            <div class="modern-card-header">
                <div class="row align-items-center">
                    <div class="col-md-6">
                        ...
                        <p class="modern-card-subtitle">View and manage your teaching sessions</p>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="bulk-actions-content">
                        <div class="bulk-actions-left">
                            ...
                            <div class="bulk-actions-buttons">
                                <button type="button" class="btn btn-sm btn-outline-success" id="bulkAccept">
                                    <i class="fas fa-check me-1"></i>Accept
                                </button>
                                <button type="button" class="btn btn-sm btn-outline-danger" id="bulkDecline">
                                    <i class="fas fa-times me-1"></i>Decline
                                </button>
                                <button type="button" class="btn btn-sm btn-outline-primary" id="bulkComplete">
                ...
                @if($bookings->count() > 0)
                    <div class="modern-table-container">
                        <table class="modern-table">
                            ...
                            <tbody>
                                @foreach($bookings as $booking)
                                    <tr class="booking-row {{ $booking->status === 'pending' ? 'pending-booking' : '' }}">
                                        ...
                                        <td>
                                            <div class="booking-datetime">
                                                ...
                                                <div class="booking-time">{{ $booking->start_time->format('g:i A') }} - {{ $booking->end_time->format('g:i A') }}</div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <a href="{{ route('teacher.bookings.show', $booking) }}" class="btn btn-sm btn-outline-primary action-btn">
                                                    <i class="fas fa-eye me-1"></i>View
                                                </a>
                                                @if($booking->status === 'pending')
                                                    <form method="POST" action="{{ route('teacher.bookings.accept', $booking) }}" class="d-inline">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-success action-btn">
                                                            <i class="fas fa-check me-1"></i>Accept
                                                        </button>
                                                    </form>
                                                    <form method="POST" action="{{ route('teacher.bookings.decline', $booking) }}" class="d-inline">
                                                        @csrf
                                                ...
                                                @if($booking->status === 'confirmed')
                                                    ...
                                                    <a href="{{ route('teacher.bookings.edit', $booking) }}" class="btn btn-sm btn-outline-warning action-btn">
                        ...
                        <div class="alert alert-info mt-3">
                            ...
                            <strong>Important:</strong> Cancellations are not allowed within {{ \App\Models\SystemSetting::getValue('cancellation_policy_hours', 24) }} hours of the scheduled time.
                        </div>
                    
                    <div class="pagination-container">
                        <div class="pagination-info">
                            Showing <strong>{{ $bookings->firstItem() }}</strong> to <strong>{{ $bookings->lastItem() }}</strong> of <strong>{{ $bookings->total() }}</strong> bookings
                        </div>
                        {{ $bookings->links() }}
                    </div>
                @else
                    <div class="empty-state">
                        ...
                        <p class="empty-state-description">You don't have any bookings yet. Students will be able to book sessions once you set your availability.</p>
                    </div>
                @endif
            </div>
        </div>
...
<style>
    ...
    .pending-booking:hover {
        background: linear-gradient(135deg, #ffeaa7 0%, #ffc107 100%);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performBulkAction(action, bookingIds) {
        ...
        // Send AJAX request
        fetch(`{{ route('teacher.bookings.bulk-actions') }}`, {
            method: 'POST',
            body: formData
        })
    ...
    bulkAcceptBtn.addEventListener('click', function() {
        performBulkAction('accept', getSelectedBookingIds());
    });
    
    bulkDeclineBtn.addEventListener('click', function() {
        performBulkAction('decline', getSelectedBookingIds());
    });
